...
def read_examples(input_file, is_training):
    ...
    examples=[]
    for val in df[['id','joke','label']].values:
    ...
    for example_index, example in enumerate(examples):
        ...
        for i in range(split_num):
            ...
            if len(context_tokens_choice) > max_seq_length-2:
                context_tokens_choice = context_tokens_choice[:max_seq_length//2-1]+context_tokens_choice[-(max_seq_length//2-1):]
...
def main():
    ...
    if args.do_train:
        ...
        for step in bar:
            ...
            # help(model)
            # loss = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, labels=label_ids).loss
            loss = model.forward(**dic)[0]
            if args.n_gpu > 1:
            ...
            if args.do_eval and (step + 1) %(args.eval_steps*args.gradient_accumulation_steps)==0:
                for file in ['dev.csv']:
                    ...
                    for input_ids, input_mask, segment_ids, label_ids in eval_dataloader:
                        ...
                        with torch.no_grad():
                            tmp_eval_loss= model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, labels=label_ids).loss
    ...
    if args.do_test:
        ...
        for file,flag in [('dev.csv','dev'),('test.csv','test')]:
            ...
            for input_ids, input_mask, segment_ids, label_ids in eval_dataloader:
                ...
                with torch.no_grad():
                    logits = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask).logits.detach().cpu().numpy()
                label_ids = label_ids.to('cpu').numpy()
            ...
            if flag=='dev':
                ...
                df['label_0']=logits[:,0]
                df['label_1']=logits[:,1]
                df[['id','label_0','label_1']].to_csv(os.path.join(args.output_dir, "dev.csv"),index=False)
